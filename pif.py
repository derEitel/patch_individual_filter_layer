...
class PatchIndividualFilters3DLoop(Module):
    ...
    def pad_to_batch_size(self, input5d):
        # expects input to be a 5D tensor!
    ...
    def split_5d(self, input_tensor):
        # get batch size of input
        bs = input_tensor.shape[0]
        # initialize
        splits = []
        # loop over batch and filter and split each "input" into patches
        for batch in range(bs):
            splits_filter = []
            for input_filter in range(self.num_local_filter_in):
                feature_map_in = input_tensor[batch, input_filter]
                # get chunks back from feature map ("input")
                t = self.get_equal_chunks(feature_map_in)
                # stack along new "chunk" dimension
                t = torch.stack(t, 0)
                # add to filter list
                splits_filter.append(t)
            # stack upon new "filter" dimension
            splits_filter = torch.stack(splits_filter, 0)
            splits.append(splits_filter)
        # stack upon new "batch" dimension
    ...
    def get_equal_chunks(self, ddd_input):
        ...
        for dim_idx, dim_size in enumerate(ddd_input_shape):
            tns_tmp = []
            for tnsr in tns:
    ...
    def forward(self, input_tensor):
        ...
        # pad
        input_tensor = self.pad_to_batch_size(input_tensor)

        # reshape
        input_tensor, bs = self.split_5d(input_tensor)

        # convolution patch wise
        patch_out = []
        for idx in range(self.num_patches):
            # get patch
            patch = input_tensor[:, :, idx]  # get each patch (3nd Dimension of 6D tensor)
        ...
        # return reassembled 5D tensor
        return self.reassemble(disassembled_out, bs)

    def reassemble(self, input6d, bs):
        # initialize output
        f_out = []
        # concat the feature maps of each patch channel back to one single map
        for batch in range(bs):
            reassambled_patches = []
            for input_filter in range(self.num_local_filter_out):
                # get patch
                patch = input6d[batch, input_filter]
                # loop over dimensions in reversed order
                for dim_idx, dim_val in reversed(list(enumerate(self.num_patches_per_dim))):
                    patch = self.reassemble_along_axis(patch, dim_idx)
                # delete now unnecessary patch dimension
                patch = patch.unsqueeze(0)
                reassambled_patches.append(patch)
            # put together all patches (now filters) along filter dimension
            f_out.append(torch.cat(reassambled_patches, 1))
        # cat together along batch dimension
        f_out = torch.cat(f_out, 0)
    ...
    def reassemble_along_axis(self, patch, dim):
        # patch is 4D tensor but dim refers to original input dimension
...
class PatchIndividualFilters3DOverlap(Module):
    def __init__(self,
                 input_dim,
                 filter_shape,
                 patch_shape,
    ...
    @staticmethod
    def get_equal_chunks_channel_last(dddc_input, tmp_num_patches_per_dim):
        dddc_input_shape = dddc_input.shape
        tns = [dddc_input]
        # for first 3 dimension in 4d input we split according to dimension and number of desired patches
        for dim_idx, dim_size in enumerate(dddc_input_shape[0:3]):
            tns_tmp = []
            for tnsr in tns:
                # determine how to split the tensor.
    ...
    def reassemble(self, input6d, bs, tmp_num_patches_per_dim):
        ...
        for batch in range(bs):
            ...
            for dim_idx, dim_val in reversed(list(enumerate(tmp_num_patches_per_dim))):
                patches = self.reassemble_along_axis(patches, dim_idx, tmp_num_patches_per_dim[dim_idx])
            # delete now unnecessary patch dimension
            f_out.append(patches.squeeze(0))
        # cat together along batch dimension
        f_out = torch.stack(f_out, 0)
    ...
    @staticmethod
    def reassemble_along_axis(patch, dim, num_patches_in_dim):
        # patch is 5D tensor but dim refers to original input dimension

        # initialize
        return_tensors = []
        # first dimension value in patch.shape determines the number of "patches"
        num_of_patches_to_put_together = patch.shape[0] // num_patches_in_dim
        # go through all "cubes" which belong together
        # if nothing was split in that dimension (==1) we don't need to concatenate anything
        if num_patches_in_dim != 1:
            start_idx = 0
            for idx in range(num_of_patches_to_put_together):
                # initialize
                tensors_to_cat_in_dim = []
                ...
                # create indices, which we want to select to cut together, assuming they are in a "row" in the array
                # these are as many as the number of tensors created during the split in that dimension
                select_idx = [start_idx] * num_patches_in_dim
                # add "offset"
                select_idx = [m1 + m2 for m1, m2 in zip(select_idx, list(range(0, num_patches_in_dim)))]
                ...
                start_idx = start_idx + num_patches_in_dim
            # cut to 5D again
            return_tensors = torch.stack(return_tensors, dim=0)
            return return_tensors
        return patch
